...
# 2) cp /usr/share/openldap-servers/DB_CONFIG.example /var/lib/ldap/DB_CONFIG (database file located in (var /lib/ldap/DB_CONFIG))
# 3) chown ldap. /var/lib/ldap/DB_CONFIG (it sets permission root to ldap)
# 4) ll /var/lib/ldap/ (check if permission is set with ldap or not)
# 5) systemctl start slapd
# 6) systemctl enable slapd
# 7) slappasswd (set password)
...
# 9) ldapadd -Y EXTERNAL -H ldapi:/// -f chrootpw.ldif
...
# 11) ldapadd -Y EXTERNAL -H ldapi:/// -f /etc/openldap/schema/nis.ldif 
# 12) ldapadd -Y EXTERNAL -H ldapi:/// -f /etc/openldap/schema/inetorgperson.ldif
			------we add schema in ldap which is already define
			---and we only add those schema which is define in ldap

# 13) vim chdomain.ldif
--->
...
# 14) ldapmodify -Y EXTERNAL -H ldapi:/// -f chdomain.ldif
...
# 18)ldapadd -x -D cn=Manager,dc=cdac,dc=in -W -f ldapuser.ldif
		--Enter LDAP Password: 
...
# 19)ldapsearch -x cn=user1 -b dc=cdac,dc=in
...
# 2) authconfig --enableldap --enableldapauth --ldapserver=192.168.20.135 --ldapbasedn="dc=cdac,dc=in" --enablemkhomedir --update
# 3) systemctl status nslcd
# 4) systemctl start nslcd
# 5) vim /etc/nsswitch.conf (if user is not visible the open this file edit ldap)
		------ passwd:     files sss ldap
	               shadow:     files sss ldap
	               group:      files sss ldap


# 6) If you get permission denied then
